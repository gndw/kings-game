...
use crate::ecs::{
    ...
    CharacterHasMother, CharacterName, CharacterOfHouse, CourtierOfCharacter, CourtierOfKingdom,
...
pub fn update(
    ...
    characters: Query<(&CharacterName, &CharacterDateOfBirth, &CharacterGender)>,
    ...
    let entries: Vec<(Entity, String, String, u32, &'static str, &'static str)> = courtiers
        ...
        .filter_map(|(_, c, role)| {
            let (name, dob, gender) = characters.get(c.0).ok()?;
            ...
            let gender_marker = match gender {
    ...
    commands.entity(courts_e).with_children(|p| {
        for (i, (char_e, name, house, char_age, gender_marker, role)) in
            entries.into_iter().enumerate()
        {
            ...
            spawn_span(p, format!(" [{}] ({})", gender_marker, char_age), Color::WHITE);
