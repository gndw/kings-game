...
//!
//! Currently just the per-holding visual primitives; the gizmo map drawing
//! itself lives in `crate::ui::map` because it shares the camera + flex
//! layout with the text panels, not because it conceptually belongs to the
//! right-hand column.
