...
| `src/map/army.rs` | on-map army indicators: lazy-spawns a sibling `ArmyIcon` entity per `Army` at the army's land holding point and keeps its `Transform` in sync each frame (so marching armies drag the icon along); reaps via `RemovedComponents<Army>`. The gizmos + name label drawing is handled by `crate::map::components::army_icon::update`, which iterates `ArmyIcon` entities — this module just owns the army ↔ icon lifecycle. |
| `src/map/components/army_icon.rs` | the army-icon visual (three yellow soldiers, white pole, waving red triangle flag, optional centred bold-white name label above the flag on a black `Sprite` background that sizes itself to the rendered text via `TextLayoutInfo`) — reusable gizmo primitive in `pub fn draw(gizmos, at, phase)`, plus `ArmyIcon` (marker), `ArmyIconLabel(String)` (label data), `ArmyIconText` (marker on the spawned text entity), `ArmyIconLabelBg(Entity)` (back-ref from bg sprite to its text), and a `PostUpdate` `update` system that draws every entity carrying the marker and lazily spawns / refreshes / reaps the label and its background |
